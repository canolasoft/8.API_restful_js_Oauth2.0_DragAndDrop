...
class Usuario
{
	...
	// Obtener un usuario autenticado a partir del token
	public function getUsuarioAutenticado($token)
	{
		$query = "SELECT usr_name, usr_email, imagen FROM usuario WHERE id = (SELECT id_usuario FROM access_token WHERE token = '$token')";
		$result = mysqli_query($this->conn, $query);
		if(!$result || mysqli_num_rows($result) == 0) {
			http_response_code(401);
			return json_encode(["error" => "Token inválido o usuario no encontrado"]);
		}else{
			$usuario = mysqli_fetch_assoc($result);
			return json_encode(["success" => [$usuario['usr_name'], $usuario['usr_email'], $usuario['imagen']]]);
		}
	}
