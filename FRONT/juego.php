<?php
include 'main_top.html';
?>
<div id="inicio" class="px-4 pt-5 my-5 text-center">
    <a class="btn btn-outline-warning mb-5" href="panel_usuario.php">
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-box-arrow-left" viewBox="0 0 16 16">
            <path fill-rule="evenodd" d="M6 12.5a.5.5 0 0 0 .5.5h8a.5.5 0 0 0 .5-.5v-9a.5.5 0 0 0-.5-.5h-8a.5.5 0 0 0-.5.5v2a.5.5 0 0 1-1 0v-2A1.5 1.5 0 0 1 6.5 2h8A1.5 1.5 0 0 1 16 3.5v9a1.5 1.5 0 0 1-1.5 1.5h-8A1.5 1.5 0 0 1 5 12.5v-2a.5.5 0 0 1 1 0z" />
            <path fill-rule="evenodd" d="M.146 8.354a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L1.707 7.5H10.5a.5.5 0 0 1 0 1H1.707l2.147 2.146a.5.5 0 0 1-.708.708z" />
        </svg>
        Regresar
    </a>
    <main>
        <table class="mx-auto">
            <tr>
                <td>
                    <span id="usr_badge" class="badge">
                        <h3 id="usr_name"></h3>
                    </span>
                </td>
                <td>
                    <strong class="text-danger mx-3"> VS </strong>
                </td>
                <td>
                    <span id="opponent_badge" class="badge">
                        <h3 id="nombre_oponente"></h3>
                    </span>
                </td>
            </tr>
            <tr>
                <td>
                    <div id="fichaO" class="draggable" draggable="true">O</div>
                </td>
                <td></td>
                <td>
                    <div id="fichaX" class="draggable" draggable="true">X</div>
                </td>
            </tr>
        </table>
        <div class="msg-container hide">
            <p id="msg">Winner</p>
        </div>
        <div class="container">
            <div class="game">
                <button class="box"></button>
                <button class="box"></button>
                <button class="box"></button>
                <button class="box"></button>
                <button class="box"></button>
                <button class="box"></button>
                <button class="box"></button>
                <button class="box"></button>
                <button class="box"></button>
            </div>
        </div>
    </main>
</div>
<!-- Fuente: https://www.geeksforgeeks.org/javascript/simple-tic-tac-toe-game-using-javascript/ -->

<?php
include 'main_bot.html';
?>

<script src="js/juego.js"></script>

</body>

</html>